...
function createServerCard(url, config) {
    ...
    card.innerHTML = `
        ...
        <div class="server-url">
            ${url}
            <button onclick="copyToClipboard('${url}')" style="background:none; border:none; color:var(--text-muted); cursor:pointer; margin-left:0.5rem;" title="Copy URL">
        ...
        <div class="server-stats">
            <div class="server-stat">
                ...
                <div class="server-stat-label">Active Models</div>
            </div>
            <div class="server-stat">
                <div class="server-stat-value">${totalRequests}</div>
                <div class="server-stat-label">Total Requests</div>
            </div>
        </div>
        ...
        <div style="display:flex; justify-content:space-between; align-items:center; font-size:0.65rem; color:var(--text-secondary); padding:0.5rem; background:var(--bg-card); border:1px solid var(--border); margin-bottom:0.75rem;">
            ...
            <div style="display:flex; align-items:center; gap:0.375rem;">
                ...
                ${config.apikey ? `<button onclick="copyToClipboard('${config.apikey}')" style="background:none; border:none; color:var(--text-muted); cursor:pointer;" title="Copy API Key"><i class="fas fa-copy"></i></button>` : ''}
            </div>
        </div>

        <!-- Models List -->
        <div class="server-models">
            <div class="server-models-title">Available Models</div>
...
async function addServer() {
    ...
    if (duplicateModels.length > 0) {
        const warningMessage = `Duplicate frontend model names found:\n${duplicateModels.join(', ')}\n\nEach frontend model name must be unique within the same server.`;
    ...
    try {
        ...
        grid.innerHTML = `
            <div class="circuit-empty">
                ...
                <span>Failed to load circuit breaker status</span>
            </div>
        `;
    }
}
    ...
    if (!circuits || Object.keys(circuits).length === 0) {
        grid.innerHTML = `
            <div class="circuit-empty">
                ...
                <span>All services are healthy, no circuit breaker events</span>
            </div>
        `;
        return;
    }
    ...
    card.innerHTML = `
        ...
        <div class="circuit-stats">
            <div class="circuit-stat">
                ...
                <div class="circuit-stat-label">Consecutive Failures</div>
            </div>
            <div class="circuit-stat">
                <div class="circuit-stat-value">${stats.total_failures}</div>
                <div class="circuit-stat-label">Total Failures</div>
            </div>
            <div class="circuit-stat">
                <div class="circuit-stat-value">${stats.total_circuit_opens}</div>
                <div class="circuit-stat-label">Circuit Opens</div>
            </div>
        </div>
        <div style="display:flex; justify-content:space-between; align-items:center; font-size:0.6rem; color:var(--text-muted); margin-top:0.5rem;">
            <span>Total Requests: ${stats.total_requests}</span>
            <span>${stats.state === 'open' ? 'Last Failure: ' + timeSinceFailure : ''}</span>
        </div>
        <div class="circuit-actions">
            <button onclick="resetCircuitBreaker('${serverKey}')" class="circuit-reset-btn" title="Reset circuit breaker">
...
async function resetCircuitBreaker(serverKey) {
    if (!confirm(`Are you sure you want to reset the circuit breaker state for server "${serverKey}"?`)) {
        return;
    }
    ...
    try {
        ...
        showNotification(`Circuit breaker for ${serverKey} has been reset`, 'success');

    } catch (error) {
        console.error('Error resetting circuit breaker:', error);
        showNotification('Failed to reset circuit breaker', 'error');
    }
}
...
async function changePassword() {
    ...
    if (newPassword.length > 72) {
        alert('Password cannot exceed 72 characters');
        return;
    }
...
function validateModelName(inputElement) {
    ...
    if (duplicateCount > 0) {
        ...
        if (!inputElement.title.includes('Duplicate')) {
            inputElement.title = `Duplicate frontend model name! ${duplicateCount} identical name(s) already exist.`;
        }
    } else {
        inputElement.title = '';
    }
